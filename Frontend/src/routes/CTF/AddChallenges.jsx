...
import { X, Plus, Upload, Tag, Lightbulb } from "lucide-react";
...
export default function AddChallenges() {
  ...
  // Initialize fetch hook in manual POST mode
    ...
    <section className="min-h-screen w-full px-4 sm:px-6 lg:px-8 xl:px-12 2xl:px-16 py-6 lg:py-10 text-green-400">
      <div className="max-w-7xl mx-auto">
        <main className="bg-black border border-green-800 p-4 sm:p-6 lg:p-8 xl:p-10 rounded-2xl shadow-2xl">
          <h1 className="text-2xl sm:text-3xl font-bold mb-6 text-green-400 border-b border-green-800 pb-4">
            Add New Challenge
          </h1>

          {/* Status Messages */}
          {loading && <p className="text-green-300">Submitting...</p>}
          {error && <p className="text-red-500">Error: {error}</p>}
          {Data && (
            <p className="text-green-500">Success! Challenge created.</p>
          )}
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-5 xl:grid-cols-3 gap-6 lg:gap-8">
                  ...
                  <div className="flex flex-col">
                    ...
                    <div className="flex flex-wrap gap-2 mb-3">
                      {form.tags.map((tag, index) => (
                        <span
                          key={index}
                          className="inline-flex items-center px-3 py-1 bg-green-800/30 border border-green-600 rounded-full text-sm text-green-200"
                        >
                          ...
                          <button
                            ...
                            className="ml-2 p-0.5 text-green-400 hover:text-red-400 transition-colors"
                          >
                            <X size={12} />
                          </button>
                        </span>
              ...
              <div className="lg:col-span-2 xl:col-span-1 space-y-6">
                <div className="bg-gray-800/30 border border-green-800 rounded-xl p-4 lg:p-6 sticky top-6">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-center gap-4 pt-6 border-t border-green-800">
              <div className="text-sm text-green-600 order-2 sm:order-1">
                ...
                <span className="inline-block">
                  Files: {form.attachments.length}
                </span>
              </div>
              <button
                type="submit"
                disabled={loading}
                className="w-full sm:w-auto px-8 py-3 bg-green-600 hover:bg-green-500 text-black font-semibold rounded-lg transition-all duration-200 transform hover:scale-105 focus:outline-none focus:ring-2 focus:ring-green-500/50 order-1 sm:order-2"
              >
                Create Challenge
              </button>
            </div>
