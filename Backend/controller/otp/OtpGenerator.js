const { User } = require("../../model/UserModel"); // Use DetailedUser for detailed registration
...
exports.OtpGenerator = async (req, res) => {
  try {
    ...
    const user = await User.findOne(request).select(
      "userDetails.email userDetails.officialEmail userDetails.regNumber _id username"
    );
    ...
    res
      .status(200)
      .json({ message: "OTP generated successfully", userId: user._id });
  } catch (error) {
